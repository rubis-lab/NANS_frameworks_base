...
public class TaskViewHeader extends FrameLayout {
    ...
    void startLaunchTaskDismissAnimation() {
        ...
        if (mDisplayButton.getVisibility() == View.VISIBLE) {
            mDisplayButton.animate().cancel();
            mDisplayButton.animate()
                    .alpha(0f)
                    .setStartDelay(0)
                    .setInterpolator(mConfig.fastOutSlowInInterpolator)
                    .setDuration(mConfig.taskViewExitToAppDuration)
    ...
    void startNoUserInteractionAnimation() {
        ...
        final boolean isNansEnabled = Settings.Global.getInt(
                mContext.getContentResolver(), Settings.Global.NANS_ENABLED, 0) == 1;
        if(isNansEnabled == false) {
            mDisplayButton.setVisibility(View.INVISIBLE);
        } else {
            mDisplayButton.setVisibility(View.VISIBLE);
            mDisplayButton.setAlpha(0f);
            mDisplayButton.animate()
                    .alpha(1f)
                    .setStartDelay(0)
                    .setInterpolator(mConfig.fastOutLinearInInterpolator)
